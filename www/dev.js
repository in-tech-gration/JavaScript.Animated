...
function initializeMethod({
  ...
  if ( options && options.more ){
    if ( options.more.length === 1 ){
      html = `
      <a class="more" href="${options.more[0].link}" target="_blank">More</a>
      `
    } else {
      // TODO: Panel with available links & resources
    }
  }
...
(function array_concat() {
  ...
  const code = `
        ...
        console.log( flatDeep ); // [ 1, 2, 3, 4, 5, 6 ];
  `
  const methodSyntax = `.concat(<span class="c-space-2x"></span>)`
  // << MODIFY
